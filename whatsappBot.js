...
const userSessions = JSON.parse(
  fs.readFileSync(userSessionsPath, "utf8") || "{}"
);

const { MongoStore } = require("wwebjs-mongo");
const mongoose = require("mongoose");
mongoose.connect(process.env.MONGO_URI_BACKUP).then(() => {
  ...
  async function handleStartSession(message, chatId, messageBody) {
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/api/whatsapp/start-session`,
        {
          code,
          userNumber: chatId,
        }
      ...
      if (response.status === 200) {
        message.reply(
          "Sesi berhasil dimulai dan telah terhubung dengan aplikasi."
        );
        ...
        await client.sendMessage(message.from, media, {
          caption:
            "Tips: Pastikan foto label nutrisi jelas, tidak gelap, tidak blur dan tidak terpotong.",
        });
      } else {
    ...
    if (userSessions[chatId] && userSessions[chatId].code) {
      ...
      try {
        const response = await axios.get(
          `${API_BASE_URL}/api/whatsapp/check-session`,
          {
            headers: {
              "x-auth-token": code,
            },
  ...
  async function handleMediaMessage(message, chatId, messageBody) {
    ...
    ━━━━━━━━━━━━━━━━━━━━`;
        ...
        if (gradeImageFile && gradeCaption) {
          const gradePhotoPath = path.join(
            __dirname,
            "public",
            "assets",
            gradeImageFile
          ...
          await client.sendMessage(message.from, gradePhoto, {
        ...
        const productName =
          messageBody.length > 1 ? messageBody : "Produk Tanpa Nama";
  ...
  async function handleHistory(message, chatId) {
    ...
    if (!userSession || !userSession.code) {
      message.reply(
        "Anda perlu masuk (start_session) untuk melihat riwayat Anda."
      );
      return;
    }
    ...
    try {
      const response = await axios.get(
        `${API_BASE_URL}/api/product/get-history`,
        {
      ...
      scanHistories.forEach((history, index) => {
        historyMessage += `${index + 1}) ${history.productName} - ${
          history.energy
        } Kalori - ${history.sugars}g Gula - *${history.grade}* grade\n`;
      });
      ...
      message.reply(
        "Terjadi kesalahan saat mengambil riwayat scan Anda. Silakan coba lagi nanti."
      );
    }
  }
  ...
  async function handleDetail(message, chatId, messageBody) {
    ...
    try {
      const response = await axios.get(
        `${API_BASE_URL}/api/product/detail-history?index=${detailNumber - 1}`,
        {
          headers: { "x-auth-token": userSession.code },
        }
      );
